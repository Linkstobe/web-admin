...
import { cn, formatDateToSequelize } from "@/lib/utils";
import {
  Command,
  CommandEmpty,
  CommandGroup,
...
export default function ReportContent() {
  ...
  const [socialMediaAccessesMetrics, setSocialMediaAccessesMetrics] =
    useState<IMetric[]>();
  const [allSocialMediaAccessesMetrics, setAllSocialMediaAccessesMetrics] =
    useState<IMetric[]>();
  ...
  const [filteredAllNewProjects, setFilteredAllNewProjects] = useState<
    IProject[]
  >([]);
  const [filteredProjects, setFilteredProjects] = useState<IProject[]>([]);
  const [selectedProject, setSelectedProject] = useState<number | null>(null);

  const [newProjectsByReferral, setNewProjectsByReferral] = useState<
    IProject[]
  >([]);
  const [filteredNewProjectsByReferral, setFilteredNewProjectsByReferral] =
    useState<IProject[]>([]);
  ...
  const onFilterProject = (value: string): void => {
    ...
    const filteredData: IProject[] = allProjects.filter(({ linkstoBe }) =>
      linkstoBe.includes(value)
    );
    ...
    const onFilterMetrics = () => {
      ...
      if (selectedProject) {
        accesses = accesses.filter(
          (metric) => Number(metric.user_id) === Number(selectedProject)
        );
        clicks = clicks.filter(
          (metric) => Number(metric.user_id) === Number(selectedProject)
        );
        socialMediaAccesses = socialMediaAccesses.filter(
          (metric) => Number(metric.user_id) === Number(selectedProject)
        );
  ...
  return (
    <div className="flex flex-col gap-4">
            ...
            <PopoverContent>
              <Command>
                <CommandInput
                  ...
                  onInput={(e) =>
                    onFilterProject((e.target as HTMLInputElement).value)
                  }
                />
                <CommandList>
      ...
      <div className="flex flex-col gap-4">
        <Separator />
        <h4 className="text-2xl font-bold tracking-tight text-[#164F62]">
      ...
      <div className="grid grid-cols-2 gap-4 bg-white rounded-lg shadow-lg p-4">
        <AccessesByOperatingSystem accessMetrics={accessesMetrics} />
      ...
      <div className="grid grid-cols-2 gap-4 bg-white rounded-lg shadow-lg p-4">
        <AccessesByState locationMetrics={locationMetrics} />

        <AccessesByCountry locationMetrics={locationMetrics} />
      </div>
