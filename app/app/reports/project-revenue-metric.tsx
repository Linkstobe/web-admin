...
export default function ProjectRevenueMetric({
  ...
  const formatedRevenue = revenue.toLocaleString("pt-BR", {
    minimumFractionDigits: 2,
    maximumFractionDigits: 2,
  });

  const [subscriptionsRevenue, setSubscriptionRevenue] =
    useState<SubscriptionsRevenueDetails>({
  ...
  useEffect(() => {
    let newProjectsCount = projects.filter(
      ({ referral_id }) => Number(referral_id) === Number(selectedProject)
    );

    if (selectedProject === null) {
      newProjectsCount = projects.filter(({ referral_id }) =>
        Number(referral_id)
      );
    } else {
      newProjectsCount = projects.filter(
        ({ referral_id }) => Number(referral_id) === Number(selectedProject)
      );
    }
    ...
    const totalRevenue =
      proSubscriptions * proSubscriptionPrice +
      premiumSubscriptions * premiumSubscriptionPrice;
    ...
    <AnalyticsSimpleCard.Root>
      <AnalyticsSimpleCard.Content>
        <AnalyticsSimpleCard.TopSection>
          <AnalyticsSimpleCard.Title title="Faturamento" />

          <AnalyticsSimpleCard.Icon icon={DollarSign} />
        </AnalyticsSimpleCard.TopSection>
        <AnalyticsSimpleCard.BodySection>
          <AnalyticsSimpleCard.Value value={`R$ ${formatedRevenue}`} />
        </AnalyticsSimpleCard.BodySection>
        <AnalyticsSimpleCard.Footer className="mt-4">
          <div className="flex flex-col gap-2">
            <div className="flex gap-4 items-center">
              ...
              <Progress
                value={subscriptionsRevenue.free.percentage}
                indicatorColor="bg-[#20B120]"
              />

              <span className="font-semibold whitespace-nowrap">
                R$ {subscriptionsRevenue.free.revenue}
              </span>
            </div>
            <div className="flex gap-4 items-center">
              ...
              <Progress
                value={subscriptionsRevenue.pro.percentage}
                indicatorColor="bg-[#164F62]"
              />

              <span className="font-semibold whitespace-nowrap">
                R$ {subscriptionsRevenue.pro.revenue}
              </span>
            </div>
            <div className="flex gap-4 items-center">
              ...
              <Progress
                value={subscriptionsRevenue.premium.percentage}
                indicatorColor="bg-[#299FC7]"
              />

              <span className="font-semibold whitespace-nowrap">
                R$ {subscriptionsRevenue.premium.revenue}
              </span>
            </div>
          </div>
